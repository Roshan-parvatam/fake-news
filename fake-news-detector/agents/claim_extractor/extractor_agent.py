...
class ClaimExtractorAgent(BaseAgent):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """Initialize claim extractor agent with configuration."""
    ...
    def _initialize_gemini_api(self) -> None:
        """Initialize Gemini API with appropriate settings."""
        try:
    ...
    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Process input for claim extraction.
        ...
        try:
            ...
            processing_time = time.time() - start_time
            self.logger.error(f"Claim extraction error: {str(e)}")
            return self.format_error_output(e, input_data)
        except Exception as e:
            processing_time = time.time() - start_time
            self.logger.error(f"Unexpected error in claim extraction: {str(e)}")
    ...
    def extract_claims(self,
        ...
        try:
            ...
            if not raw_extraction or raw_extraction.startswith("Content"):
                # Fallback for blocked content
                fallback_prompt = f"List main factual claims from: {article_text[:500]}"
                try:
                    response = self.model.generate_content(fallback_prompt)
                    if self._is_valid_response(response):
                        raw_extraction = response.candidates[0].content.parts[0].text
                except Exception:
                    raw_extraction = "Error: Unable to extract claims due to content restrictions"

            # Step 3: Parse extracted claims
            ...
            if len(structured_claims) > self.max_claims:
                ...
                self.logger.info(f"Limited claims to maximum: {self.max_claims}")
            ...
            if self.enable_prioritization and structured_claims:
                prioritization_analysis = self._generate_claim_prioritization(structured_claims)
            ...
            self.logger.info(f"Successfully extracted {len(structured_claims)} claims in {processing_time:.2f}s")
            return result
    ...
    def _generate_claim_extraction(self, article_text: str, prediction: str,
        ...
        """Generate AI-powered claim extraction."""
        try:
            ...
            if not self._is_valid_response(response):
                self.logger.warning("Claim extraction blocked by safety filters")
                raise LLMResponseError("Content blocked by safety filters", "claim_extraction", self.model_name)

            return response.candidates[0].content.parts[0].text

        except Exception as e:
            self.logger.error(f"Error generating claim extraction: {str(e)}")
    ...
    def _generate_verification_analysis(self, structured_claims: List[Dict]) -> str:
        """Generate verification analysis for extracted claims."""
        try:
            ...
            self.logger.error(f"Error generating verification analysis: {str(e)}")
    ...
    def _generate_claim_prioritization(self, structured_claims: List[Dict]) -> str:
        """Generate claim prioritization analysis."""
        try:
            claims_text = "\n".join([
            ...
            prompt = get_claim_prompt_template(
                ...
                extracted_claims=claims_text,
                domain='general'  # Could be parameterized
            )
            ...
            self.logger.error(f"Error generating prioritization: {str(e)}")
    ...
    def validate_input(self, input_data: Dict[str, Any]) -> tuple[bool, str]:
        """Validate input data for claim extraction."""
...
if __name__ == "__main__":
    ...
    try:
        ...
        quick_claims = agent.quick_extract(test_input['text'], max_claims=3)
        ...
        print(f"   Success rate: {(metrics['extraction_specific_metrics']['successful_extractions'] / max(metrics['extraction_specific_metrics']['total_extractions'], 1)) * 100:.1f}%")
